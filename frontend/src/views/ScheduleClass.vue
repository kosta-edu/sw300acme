                ...
                <div slot="header">
                    <h3>
                        ...
                        <v-chip label outline color="primary">
                            <v-icon left>supervisor_account</v-icon>{{ item.evaluationRate }} %
                        </v-chip>
                    </h3>
                </div>
                <v-card>
                    <v-card-text v-if="item.classDayList.length > 0">
                        <v-card
                            v-for="(classes, j) in item.classDayList"
                            :key="j">
                            <v-list subheader style="padding: 15px;">
                                <h2>Number : {{ classes.number }}</h2>
                                    Date : {{ classes.date | moment("YYYY-MM-DD") }}<br />
                                    Start time : {{ classes.startTime | moment("HH:mm:ss") }}<br />
                                    End time : {{ classes.endTime | moment("HH:mm:ss") }}<br />
                                    Instructor pay : {{ classes.instructorPay }}<br />
                            </v-list>
                            <v-card-actions>
    ...
    export default {
        ...
        created() {
            ...
            if (idxArr.length > 0) {
                ...
                axios.get('http://localhost:8080/schedule-service/course-clazzes?courseId=' + this.courseId)
                    .then((res) => {
